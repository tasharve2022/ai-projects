...
import type { FlatList, HostInstance, ScrollView, SectionList, TextStyle, TransformsStyle, ViewStyle } from 'react-native';
 ...
 */
type SharedValueDisableContravariance<Value = unknown> = Omit<SharedValue<Value>, 'set' | 'modify'>;
...
type ReanimatedCSSProps = Partial<CSSAnimationProperties & CSSTransitionProperties>;
type WithReanimatedCSS<Style> = (Style & ReanimatedCSSProps) | (Style extends object ? Omit<Style, keyof ReanimatedCSSProps> & ReanimatedCSSProps : never);
export type AnimatedStyle<Style = DefaultStyle> = WithReanimatedCSS<Style> | MaybeSharedValueRecursive<Style> | AnimatedStyleHandle<Style>;
...
type ScrollResponderType = InternalHostInstance & Partial<ReturnType<NonNullable<GetProp<ScrollView, 'getScrollResponder'> | GetProp<FlatList, 'getScrollResponder'> | GetProp<SectionList, 'getScrollResponder'>>> & JSX.Element>;
export type InternalHostInstance = Partial<HostInstance & {
    getScrollResponder: () => Maybe<ScrollResponderType>;
    getNativeScrollRef: () => Maybe<Partial<InternalHostInstance & typeof ScrollView>>;
